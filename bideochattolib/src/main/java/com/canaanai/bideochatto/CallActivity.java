...
public class CallActivity extends Activity implements AppRTCClient.SignalingEvents,
    ...
    public static final String EXTRA_ROOM_SERVER = "ROOMSERVER";
    public static final String EXTRA_USER_ID = "USERID";
    public static final String EXTRA_USER_NAME = "USERID";
    ...
    //是否处于对话框状态
    private boolean isDialogState = false;
    private AlertDialog dialog1;//网络异常对话框
    private AlertDialog dialog2;//网络异常对话框
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (commandLineRun && runTimeMs > 0) {
            (new Handler()).postDelayed(new Runnable() {
                @Override
                public void run() {
                    Log.e(TAG, "onCreate方法调用disconnect()");
                    disconnect();
                }
            }, runTimeMs);
        }
    ...
    protected void onDestroy() {
        unRegisterHungUpReceiver();
        Log.e(TAG, "onDestroy方法调用disconnect()");
        disconnect();
        if (logToast != null) {
            logToast.cancel();
        }
    ...
    public void onCallHangUp() {
        Log.e(TAG, "onCallHangUp方法调用disconnect()");
        isDialogState = false;
        disconnect();
    }
    ...
    private void disconnectWithErrorMessage(final String errorMessage) {
        if (commandLineRun || !activityRunning) {
            ...
            disconnect();
            Log.e(TAG, "disconnectWithErrorMessage方法调用disconnect()");
    ...
    public void onChannelClose() {
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                Log.e(TAG, "onChannelClose方法调用disconnect()");
//        disconnect();
                if (isDialogState == false) {
                    isDialogState = true;
                    Intent endCallIntent = new Intent();
                    endCallIntent.setAction("com.unisky.endCall");
                    CallActivity.this.sendBroadcast(endCallIntent);//发送关闭心跳的广播
                    dialog1 = new AlertDialog.Builder(CallActivity.this)
                            .setTitle("提示")
                            .setMessage("网络异常")
                            .setCancelable(false)
                            .setNeutralButton("确定",
    ...
    public void onIceDisconnected() {
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                iceConnected = false;
                Log.e(TAG, "onIceDisconnected方法调用disconnect()");
//        disconnect();
                isDialogState = true;
                Intent endCallIntent = new Intent();
                endCallIntent.setAction("com.unisky.endCall");
                CallActivity.this.sendBroadcast(endCallIntent);//发送关闭心跳的广播
                dialog2 = new AlertDialog.Builder(CallActivity.this)
                        .setTitle("提示")
                        .setMessage("通话结束")
                        .setCancelable(false)
                        .setNeutralButton("确定",
...
//    private LocalBroadcastManager lbm;
    ...
    public static final String ACTION_HUNG_UP = "com.unisky.action.hungup";

    private void registerHungUpReceiver() {
        IntentFilter intentFilter = new IntentFilter();
        intentFilter.addAction(ACTION_HUNG_UP);
        mReceiver = new BroadcastReceiver() {
            @Override
            public void onReceive(Context context, Intent intent) {
//                onCallHangUp();
                String action = intent.getAction();
                if (TextUtils.equals(action,ACTION_HUNG_UP)){
                    isDialogState = false;
                    disconnect();
                    Log.e(TAG, "收到关闭广播com.unisky.action.hungup");
                }
            }
        };

//        lbm = LocalBroadcastManager.getInstance(this);
//        lbm.registerReceiver(mReceiver, intentFilter);
        this.registerReceiver(mReceiver, intentFilter);
    }


    private void unRegisterHungUpReceiver() {
